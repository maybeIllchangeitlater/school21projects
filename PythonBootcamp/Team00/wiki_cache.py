...
def main():
    ...
    start_url =  'https://en.wikipedia.org/wiki/' + str(options.PAGENAME) if options.PAGENAME else 'https://en.wikipedia.org/wiki/Python_(' \
...
def make_link_soup(req: requests.Response, headlink: str):
    soup = BeautifulSoup(req.text, 'lxml')
    link_soup = []
    for link in soup.find(id="bodyContent").find_all('a', href=True):
        if link['href'].startswith('/wiki/'):
            fixed_link = unzashquar_links(link['href'])
            if len(link_soup) > 10:
                return link_soup
